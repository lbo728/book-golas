...
| 환경 | URL |
|------|-----|
| Production | https://web-pv2r0fv7v-lbo728s-projects.vercel.app/admin |
...
특정 사용자에게 테스트 푸시 알림을 즉시 발송할 수 있습니다.
...
1. **대상 사용자 선택**
   - FCM 토큰이 등록된 사용자 목록에서 선택
   - 사용자 카드를 클릭하거나 드롭다운에서 선택

2. **메시지 템플릿 선택**
   - **커스텀 메시지**: 직접 제목/본문 입력
   - **기존 템플릿**: inactive, deadline, progress, streak, achievement 중 선택

3. **미리보기 확인**
   - 우측 미리보기 카드에서 실제 푸시 모양 확인
   - 발송 정보 (타입, 대상, 디바이스 수) 확인
...
- 템플릿 변수(`{days}`, `{bookTitle}` 등)는 실제 값으로 치환되지 않음
- 테스트 발송도 `push_logs`에 `test` 타입으로 기록됨
- 알림이 비활성화된 사용자는 목록에 표시되지 않음

### 발송 결과

| 결과 | 설명 |
|------|------|
| 발송 성공 | 지정된 디바이스로 푸시 전송 완료 |
| 발송 실패 | FCM 토큰 만료 또는 네트워크 오류 |
